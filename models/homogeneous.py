...
def reynolds ( Flow_info, line) -> float:
    return  ((Flow_info.mix_rho*Flow_info.vm*line.Dh)/Flow_info.mix_viscosity)

def F( Flow_info, line ) -> float:

    def f(F):
        return ((- 2*np.log10( ( (line.e/line.Dh) / (3.7) ) + ( 2.51 / ( reynolds(Flow_info,line)*np.sqrt(F) ) ))) - ( 1 / np.sqrt(F) )) 

    F = fsolve(f,0.001)[0]
    return F

def frictional_gradient_homo(Flow_info, line) -> float:
    return ( F(Flow_info, line )*( (Flow_info.mix_rho*Flow_info.vm**2)/(2*line.Dh) ) )
...
def kinetic_dl_t(Flow_info, line) -> float: # em função de Dp/dl total
    A = ((Flow_mass_mix_homo(Flow_info))**2) / (line.area**2)
