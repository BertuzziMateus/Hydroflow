...
def drift_infos_bha( Flow_info, tubing, ) -> list[float]:
...
def reynolds_bha ( Flow_info, tubing ) -> float:
    vsg,vsl,vm,Hl,mix_rho,mix_viscosity,C0,vd = drift_infos_bha(Flow_info,tubing)
    return  ((mix_rho*vm*tubing.Dh)/mix_viscosity)

def F_bha( Flow_info, tubing,  )  -> float:
    re = reynolds_bha(Flow_info,tubing)
    def f(F):
...
def mass_flow_mix_bha( Flow_info, tubing) -> float:
    Hl = drift_infos_bha(Flow_info,tubing)[3]
    return ( (Flow_info.flow_liquid_mass * Hl) + Flow_info.flow_gas_mass*(1-Hl) )

def titulo_bha( Flow_info, tubing) -> float:
    return (Flow_info.flow_gas_mass/mass_flow_mix_bha(Flow_info,tubing))

def frictional_gradient_Bha( Flow_info, tubing) -> float:
    vsg,vsl,vm,Hl,mix_rho,mix_viscosity,C0,vd = drift_infos_bha(Flow_info,tubing)
    return ( F_bha( Flow_info , tubing)*( (mix_rho*vm**2)/(2*tubing.Dh) ) )
...
def ek_Bha( Flow_info, tubing) -> float: 
    A = (mass_flow_mix_bha(Flow_info,tubing)**2) / (tubing.area**2)
    B = (Flow_info.MM) / (8.314462*Flow_info.T*Flow_info.gas_rho**2)
    ekk = (A*titulo_bha(Flow_info,tubing)*B) 
    ekk = 0 
    return ekk
