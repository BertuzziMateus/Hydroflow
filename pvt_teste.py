...
while P >= 1 :
    ...
    if P == 350:
        print(PB_lasater(fluid))
        print(dead_viscosity_beggs(fluid))

    x.append(P)
    y.append(oil_density(fluid))
    y1.append(Oil_Density_standing(fluid))
    P -= 1

plt.plot(x, y, label='Lasater')
plt.plot(x, y1,  label='Standing')
plt.legend()
plt.show()
...
#             Bg = Bg(fluid), # scf/cf
#             Rs = Rs_lasater(fluid)*0.17810760667903525,  # sm^3 / m^3 - #scf/STb
